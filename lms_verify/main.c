...
static iqr_retval showcase_lms_verify(const iqr_Context *ctx, const uint8_t *security, const size_t security_size,
    const iqr_LMSWinternitz w, const iqr_LMSHeight height, const uint8_t *digest, const char *pub_file, const char *sig_file)
{
...
static void preamble(const char *cmd, const char *security, const char *sig, const char *pub, const iqr_LMSWinternitz w,
...
static iqr_retval parse_commandline(int argc, const char **argv, const char **security, const char **sig, const char **pub,
    iqr_LMSWinternitz *w, iqr_LMSHeight *height, const char **message)
{
    int i = 1;
    while (i != argc) {
        if (i + 2 > argc) {
...
int main(int argc, const char **argv)
{
     ...
     * an error.
     */
    iqr_retval ret = parse_commandline(argc, argv, &security, &sig, &pub, &w, &height, &message);
    if (ret != IQR_OK) {
        return EXIT_FAILURE;
    }

    /* Make sure the user understands what we are about to do. */
    preamble(argv[0], security, sig, pub, w, height, message);
     ...
     */
    ret = showcase_lms_verify(ctx, (const uint8_t *)security, strlen(security), w, height, digest, pub, sig);
