...
static iqr_retval showcase_lms_sign(const iqr_Context *ctx, const iqr_RNG *rng, const iqr_LMSWinternitz w,
    const iqr_LMSHeight height, const uint8_t *digest, const char *priv_file, uint32_t index, const char *sig_file)
{
    ...
    ret = iqr_LMSImportPrivateKey(params, priv_raw, priv_raw_size, index, &priv);
    if (ret != IQR_OK) {
...
{
    fprintf(stdout, "lms_sign --index <number> \n"
...
static void preamble(const char *cmd, const char *sig, const char *priv, const iqr_LMSWinternitz w, const iqr_LMSHeight height,
...
int main(int argc, const char **argv)
{
     ...
     * an error.
     */
    iqr_retval ret = parse_commandline(argc, argv, &sig, &priv, &index, &w, &height, &message);
    if (ret != IQR_OK) {
        return EXIT_FAILURE;
    }

    /* Make sure the user understands what we are about to do. */
    preamble(argv[0], sig, priv, w, height, index, message);
     ...
     */
    ret = showcase_lms_sign(ctx, rng, w, height, digest, priv, index, sig);
