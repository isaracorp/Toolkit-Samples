/** @file main.c
 *
 * @brief Generate keys using the toolkit's LMS signature scheme.
 *
...
static iqr_retval showcase_lms_keygen(const iqr_Context *ctx, const iqr_RNG *rng, const char *pub_file, const char *priv_file,
    const iqr_LMSWinternitz w, const iqr_LMSHeight height)
{
    ...
    /* Generate the keys. */
    ret = iqr_LMSCreateKeyPair(params, rng, IQR_LMS_FULL_TREE, &pub, &priv);
    if (ret != IQR_OK) {
...
{
    fprintf(stdout, "lms_generate_keys [--pub <filename>]\n"
        "  [--priv <filename>] [--winternitz 1|2|4|8] [--height 5|10|15|20|25]\n");
...
static void preamble(const char *cmd, const char *pub, const char *priv, const iqr_LMSWinternitz w, const iqr_LMSHeight height)
{
...
static iqr_retval parse_commandline(int argc, const char **argv, const char **pub, const char **priv, iqr_LMSWinternitz *w,
    iqr_LMSHeight *height)
{
    int i = 1;
    while (i != argc) {
        if (i + 2 > argc) {
...
int main(int argc, const char **argv)
{
     ...
     * an error.
     */
    iqr_retval ret = parse_commandline(argc, argv, &pub, &priv, &w, &height);
    if (ret != IQR_OK) {
        return EXIT_FAILURE;
    }

    /* Make sure the user understands what we are about to do. */
    preamble(argv[0], pub, priv, w, height);
     ...
     */
    ret = showcase_lms_keygen(ctx, rng, pub, priv, w, height);
