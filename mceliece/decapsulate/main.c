/** @file main.c Demonstrate the toolkit's McEliece QC-MDPC Key Encapsulation Mechanism.
 *
...
// ---------------------------------------------------------------------------------------------------------------------------------
// This function showcases McEliece QC-MDPC decapsulation.
// ---------------------------------------------------------------------------------------------------------------------------------

static iqr_retval showcase_mceliece_decapsulation(const iqr_McElieceParams *params, const char *privkey_file,
    const char * ciphertext_file, const char *sharedkey_file)
{
    ...
    /* Perform McEliece QC-MDPC decapsulation. */
    ret = iqr_McElieceDecapsulate(privkey, ciphertext, ciphertext_size, sharedkey, sizeof(sharedkey));
    if (ret != IQR_OK) {
        fprintf(stderr, "Failed on iqr_McElieceDecapsulate(): %s\n", iqr_StrError(ret));
        goto end;
    }
    ...
    fprintf(stdout, "McEliece QC-MDPC decapsulation completed.\n");
...
static void usage(void)
{
    ...
    fprintf(stdout, "mceliece_decapsulate [--priv <filename>] [--ciphertext <filename>]\n [--shared <filename>]\n");
...
int main(int argc, const char **argv)
{
    ...
    /* Showcase McEliece QC-MDPC decapsulation. */
    ret = showcase_mceliece_decapsulation(parameters, private_key_file, ciphertext_file, sharedkey_file);
