 ...
 */
static iqr_FrodoParams *params;
static iqr_FrodoInitiatorPrivateKey *initiator_private_key;

iqr_retval init_alice(const iqr_Context *ctx, const iqr_FrodoVariant *variant)
{
    if (ctx == NULL) {
    ...
    iqr_retval ret = iqr_FrodoCreateParams(ctx, variant, &params);
    if (ret != IQR_OK) {
        fprintf(stderr, "Failed on iqr_FrodoCreateParams(): %s\n", iqr_StrError(ret));
    }
    return ret;
}
...
iqr_retval alice_start(const iqr_RNG *rng, bool dump)
{
    ...
    iqr_retval ret = iqr_FrodoGetInitiatorPublicKeySize(params, &initiator_size);
    if (ret != IQR_OK) {
        fprintf(stderr, "Failed on iqr_FrodoGetInitiatorPublicKeySize(): %s\n", iqr_StrError(ret));
        return ret;
    }
    ...
    ret = iqr_FrodoCreateInitiatorPrivateKey(params, rng, &initiator_private_key);
    if (ret != IQR_OK) {
        fprintf(stderr, "Failed on iqr_FrodoCreateInitiatorPrivateKey(): %s\n", iqr_StrError(ret));
        goto end;
    }

    ret = iqr_FrodoGetInitiatorPublicKey(initiator_private_key, rng, initiator_public_key, initiator_size);
    if (ret != IQR_OK) {
        fprintf(stderr, "Failed on iqr_FrodoGetInitiatorPublicKey(): %s\n", iqr_StrError(ret));
        goto end;
    }
    ...
    if (ret != IQR_OK) {
        iqr_FrodoDestroyInitiatorPrivateKey(&initiator_private_key);
    }
...
iqr_retval alice_get_secret(uint8_t *secret, size_t secret_size)
{
    ...
    if (secret == NULL || secret_size != IQR_FRODO_SECRET_SIZE) {
    ...
    ret = iqr_FrodoGetResponderPublicKeySize(params, &responder_size);
    if (ret != IQR_OK) {
        fprintf(stderr, "Failed on iqr_FrodoGetResponderPublicKeySize(): %s\n", iqr_StrError(ret));
        goto end;
    }
    ...
    ret = iqr_FrodoGetInitiatorSecret(initiator_private_key, responder_public_key, responder_size, secret, secret_size);
    if (ret != IQR_OK) {
        fprintf(stderr, "Failed on iqr_FrodoGetInitiatorSecret(): %s\n", iqr_StrError(ret));
        goto end;
    }
...
{
    iqr_retval ret = iqr_FrodoDestroyParams(&params);
    if (ret != IQR_OK) {
        fprintf(stderr, "Failed on iqr_FrodoDestroyParams(): %s\n", iqr_StrError(ret));
    }
    return ret;
}
