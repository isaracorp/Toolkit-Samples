/** @file main.c Demonstrate the toolkit's Frodo implementation.
 *
...
// ---------------------------------------------------------------------------------------------------------------------------------
// This function showcases the use of the Frodo algorithm to generate a
// shared secret.
//
...
static iqr_retval showcase_frodo(const iqr_Context *ctx, const iqr_RNG *rng, bool dump, const iqr_FrodoVariant *variant)
{
    ...
    uint8_t alice_secret[IQR_FRODO_SECRET_SIZE] = { 0 };
    uint8_t bob_secret[IQR_FRODO_SECRET_SIZE] = { 0 };
     ...
     * the Frodo data protocol works see the README.md.
     */
...
{
    fprintf(stdout, "frodo [--dump] [--variant recommended|paranoid]\n");
    ...
    fprintf(stdout, "        --variant The variant of Frodo to use.\n");
...
static void preamble(const char *cmd, bool dump, const iqr_FrodoVariant *variant)
{
    ...
    if (variant == &IQR_FRODO_RECOMMENDED) {
        fprintf(stdout, "Recommended\n");
    } else if (variant == &IQR_FRODO_PARANOID) {
        fprintf(stdout, "Paranoid\n");
    } else {
...
static iqr_retval parse_commandline(int argc, const char **argv, bool *dump, const iqr_FrodoVariant **variant)
{
    int i = 1;
        ...
        if (paramcmp(argv[i], "--dump") == 0) {
            *dump = true;
            ...
            if (paramcmp(argv[i], "recommended") == 0) {
                *variant = &IQR_FRODO_RECOMMENDED;
            } else if (paramcmp(argv[i], "paranoid") == 0) {
                *variant = &IQR_FRODO_PARANOID;
            } else {
                usage();
                return IQR_EBADVALUE;
            }
...
int main(int argc, const char **argv)
{
    ...
    iqr_RNG *rng = NULL;
    bool dump = false;
    const iqr_FrodoVariant *variant = &IQR_FRODO_RECOMMENDED;
    ...
    /* IQR initialization that is not specific to Frodo. */
    ret = init_toolkit(&ctx, &rng);
    if (ret != IQR_OK) {
        goto cleanup;
    }

    /* This function showcases the usage of Frodo. */
    ret = showcase_frodo(ctx, rng, dump, variant);
