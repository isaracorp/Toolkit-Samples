...
static iqr_FrodoResponderPrivateKey *responder_private_key;

iqr_retval init_bob(const iqr_Context *ctx, const iqr_FrodoVariant *variant)
{
    if (ctx == NULL) {
...
iqr_retval bob_start(const iqr_RNG *rng, bool dump)
{
    ...
    ret = iqr_FrodoCreateResponderPrivateKey(params, rng, &responder_private_key);
    if (ret != IQR_OK) {
        fprintf(stderr, "Failed on iqr_FrodoCreateResponderPrivateKey(): %s\n", iqr_StrError(ret));
        goto end;
    }

    ret = iqr_FrodoGetResponderPublicKey(responder_private_key, rng, initiator_public_key, initiator_size, responder_public_key,
        responder_size);
    if (ret != IQR_OK) {
        fprintf(stderr, "Failed on iqr_FrodoGetResponderPublicKey(): %s\n", iqr_StrError(ret));
        goto end;
    }
    ...
    if (ret != IQR_OK) {
        iqr_FrodoDestroyResponderPrivateKey(&responder_private_key);
    }
...
iqr_retval bob_get_secret(uint8_t *secret, size_t secret_size)
{
    ...
    ret = iqr_FrodoGetResponderSecret(responder_private_key, secret, secret_size);
    if (ret != IQR_OK) {
        fprintf(stderr, "Failed on iqr_FrodoGetResponderSecret(): %s\n", iqr_StrError(ret));
        goto end;
    }
