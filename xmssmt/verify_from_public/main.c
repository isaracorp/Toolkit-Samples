/** @file main.c
 *
 * @brief Get parameters from a public key, then verify a signature using the
 * toolkit's XMSS^MT signature scheme.
 *
...
"xmssmt_verify_from_public [--sig <filename>] [--pub <filename>]\n"
"  [--message <filename>]\n"
"\n"
"    Defaults:\n"
"        --sig sig.dat\n"
...
// an XMSS^Mt signature against a digest.
// ---------------------------------------------------------------------------------------------------------------------------------

static iqr_retval showcase_xmssmt_verify_from_public(const iqr_Context *ctx, const uint8_t *digest, const char *pub_file,
    const char *sig_file)
{
    ...
    ret = iqr_XMSSMTCreateParamsFromPublicKeyBuffer(ctx, pub_raw, pub_raw_size, &params);
    if (ret != IQR_OK) {
        fprintf(stderr, "Failed on iqr_XMSSMTCreateParamsFromPublicKeyBuffer(): %s\n", iqr_StrError(ret));
        goto end;
    }
...
int main(int argc, const char **argv)
{
    ...
    ret = showcase_xmssmt_verify_from_public(ctx, digest, pub, sig);
