...
static iqr_retval showcase_chacha20_encrypt(const uint8_t *key_data, size_t key_size, const uint8_t *nonce_data, size_t nonce_size,
    uint32_t counter, const uint8_t *plaintext_data, size_t plaintext_size, const char *ciphertext)
{
    size_t ciphertext_size = plaintext_size;
    uint8_t *ciphertext_data = calloc(1, ciphertext_size);
    if (ciphertext_data == NULL) {
        fprintf(stderr, "Failed on calloc(): %s\n", strerror(errno));
        return IQR_ENOMEM;
    }

    iqr_retval ret = iqr_ChaCha20Encrypt(key_data, key_size, nonce_data, nonce_size, counter, plaintext_data, plaintext_size,
    ...
    fprintf(stdout, "ChaCha20 encrypt completed.\n");

    ret = save_data(ciphertext, ciphertext_data, ciphertext_size);
    if (ret != IQR_OK) {
        goto end;
    }

    fprintf(stdout, "Ciphertext has been saved to disk.\n");
...
{
    fprintf(stdout, "chacha20_encrypt [--key <filename>] [--nonce <filename>]\n"
...
int main(int argc, const char **argv)
{
    ...
    preamble(argv[0], key, nonce, counter, plaintext, ciphertext);
